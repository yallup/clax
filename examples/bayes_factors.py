...
def plot(a, lr, true_k, network_k, true_k_m2, network_k_m2):
    a.scatter(
        true_k_m2,
        network_k_m2,
        alpha=0.5,
    ...
    a.set_title(f"lr: {lr:.2e}")
    # a.suptitle(f"lr: {lr:.2e}")
...
# for i, lr in enumerate([2e-4,1e-4,5e-5]):
...
#     true_k_m2 = M_1.logpdf(X_test_2) - M_0.logpdf(X_test_2)
#     network_k_m2 = classifier.predict(X_test_2).squeeze()
#     plot(a[i], lr, true_k, network_k, true_k_m2, network_k_m2)

# f.tight_layout()
# f.savefig("en_exp.pdf")
